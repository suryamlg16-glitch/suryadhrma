...
<nav class="bg-[#B08968] shadow-lg sticky top-0 z-50">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-20"> {{-- Ubah dari h-24 ke h-20 --}}
            <!-- Logo dengan efek hover -->
            <div class="flex-shrink-0 flex items-center group">
                <a href="{{ route('beranda') }}" class="relative">
                    <img src="{{ asset('images/logomebel.png') }}" 
                         alt="Logo Mebel" 
                         class="h-14 w-auto object-contain brightness-0 invert transform transition-transform duration-300 group-hover:scale-105"> {{-- Ubah dari h-20 ke h-14 --}}
            ...
            <!-- Desktop Menu dengan efek modern (background slide dari kiri) - TANPA BULLET POINT -->
            <div class="hidden md:flex items-center space-x-0.5"> {{-- Ubah dari space-x-1 ke space-x-0.5 --}}
                <a href="{{ route('beranda') }}" 
                   class="relative text-white px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300 group overflow-hidden"> {{-- Ubah dari px-5 py-2.5 text-base ke px-4 py-2 text-sm --}}
            ...
            <!-- Social Media Icons - Clean & Modern -->
            <div class="hidden md:flex items-center space-x-2"> {{-- Ubah dari space-x-3 ke space-x-2 --}}
                <!-- Instagram -->
            ...
            <div class="md:hidden flex items-center space-x-2"> {{-- Ubah dari space-x-3 ke space-x-2 --}}
                <a href="#" class="text-white relative group">
                    <svg class="w-6 h-6 transform transition-transform duration-300 group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24"> {{-- Ubah dari w-7 h-7 ke w-6 h-6 --}}
    ...
    <!-- Mobile Search -->
    <div class="md:hidden px-4 pb-3"> {{-- Ubah dari pb-4 ke pb-3 --}}
        <div class="relative group">
            <input type="text" 
                   placeholder="Cari produk..." 
                   class="w-full bg-white/10 text-white placeholder-white/70 rounded-full pl-12 pr-5 py-2.5 focus:outline-none focus:ring-2 focus:ring-white/50 border border-white/20 text-sm transition-all duration-300 group-hover:bg-white/20 focus:bg-white/20"> {{-- Ubah dari py-3 text-base ke py-2.5 text-sm --}}
            <svg class="w-4 h-4 text-white/70 absolute left-4 top-3 transition-transform duration-300 group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24"> {{-- Ubah dari w-5 h-5 top-3.5 ke w-4 h-4 top-3 --}}
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
            </svg>
        </div>
    </div>
    
    <!-- Mobile Menu dengan efek slide dan social media links -->
    <div id="mobile-menu" class="hidden md:hidden bg-[#8B6F4F] transform transition-all duration-300 origin-top">
        <div class="px-2 pt-2 pb-3 space-y-0.5"> {{-- Ubah dari space-y-1 ke space-y-0.5 --}}
            <a href="{{ route('beranda') }}" class="block px-3 py-2 text-white hover:bg-[#B08968] rounded-md transition-all duration-300 transform hover:translate-x-2 text-sm">BERANDA</a> {{-- Ubah dari py-3 ke py-2, tambah text-sm --}}
            ...
            <!-- Social Media Links di Mobile Menu -->
            <div class="pt-3 pb-2 border-t border-white/20 mt-3"> {{-- Ubah dari pt-4 pb-2 mt-4 ke pt-3 pb-2 mt-3 --}}
                <p class="text-white/70 text-xs px-4 pb-2">Follow Kami</p>
                <div class="flex items-center space-x-2 px-4">
                    <a href="https://instagram.com" 
                       target="_blank" 
                       class="text-white/90 hover:text-white bg-white/10 p-2 rounded-full transition-all duration-300"> {{-- Ubah dari p-2.5 ke p-2 --}}
                        <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"> {{-- Ubah dari w-4 h-4 ke w-3.5 h-3.5 --}}
                            <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.76 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
                        </svg>
                    </a>
                    <a href="https://tiktok.com" 
                       target="_blank" 
                       class="text-white/90 hover:text-white bg-white/10 p-2 rounded-full transition-all duration-300">
